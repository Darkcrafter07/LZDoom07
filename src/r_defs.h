...
class GLWall;          // Forced-Perspective Anamorphic sprite clipping
struct extsector_t
{
	...
	//Forced-Perspective Anamorphic sprite clipping walls
	TArray<GLWall*> Walls;
};
...
public:
	...
	bool isClosedWithTolerance(double tolerance = 8.0) const
	{
		DVector3 floorNormal = floorplane.Normal();
		DVector3 ceilNormal = -ceilingplane.Normal();

		double angleDiff = acos(clamp<double>(floorNormal.X * ceilNormal.X + floorNormal.Y * ceilNormal.Y + floorNormal.Z * ceilNormal.Z, -1.f, 1.f)) * (180.0 / M_PI);
		double distDiff = abs(floorplane.D - (-ceilingplane.D));

		return (angleDiff < 5.0f) && (distDiff <= tolerance);
	}
