...
void AActor::UpdateRenderSectorList()
{
	...
	if (Pos() != OldRenderPos && !(flags & MF_NOSECTOR))
	{
		...
		{
			// [SAFETY CHECK] PortalBlockmap dimensions
			if (PortalBlockmap.dx <= 0 || PortalBlockmap.dy <= 0)
				return;  // Not initialized

			int bx = level.blockmap.GetBlockX(X());
			int by = level.blockmap.GetBlockY(Y());
			FBoundingBox bb(X(), Y(), MIN(radius * 1.5, 128.));

			// Validate block coordinates against PortalBlockmap dimensions
			if (bx >= 0 && bx < PortalBlockmap.dx &&
				by >= 0 && by < PortalBlockmap.dy)
			{
				const auto &block = PortalBlockmap(bx, by);
				if (block.neighborContainsLines)
				{
